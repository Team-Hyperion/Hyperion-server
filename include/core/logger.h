...
#ifndef HYPERION_INCLUDE_CORE_LOGGER_H
#define HYPERION_INCLUDE_CORE_LOGGER_H
#pragma once
...
#define LOG_MESSAGE(severity__, format__) \
    hyperion::core::MakeLogMessage<hyperion::core::LogSeverity::severity__>(format__, FILENAME, __LINE__)

// Allows the message to contain a format, similar to printf
#define LOG_MESSAGE_F(severity__, format__, ...) \
    hyperion::core::MakeLogMessage<hyperion::core::LogSeverity::severity__>(format__, FILENAME, __LINE__, __VA_ARGS__)
...
namespace hyperion::core
{
    ...
    void MakeLogMessage(const char* format, const char* file, const int line, Args&&... args) {
        if constexpr (static_cast<int>(Severity) >= HYPERION_LOG_LEVEL) {
...
#endif // HYPERION_INCLUDE_CORE_LOGGER_H
